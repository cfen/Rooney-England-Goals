...
function buildLayout (dataIn,filteredData){
	...
	var shareButtonsStr='<div id="shareButtons"><button data-source="twitter" id="shareTwitter" data-player="iansarah"></button><button data-source="facebook" id="shareFacebook" data-player="iansarah"></button></div>'
	var lhColStr = '<div class="sub-left-col-copy"></div>'

	var cardString = '<div class="subContentBlock"><h4 class="subSectionHeading">Player Name</h4>'+lhColStr+'<div class="pitchHolder">'+pitchSVG+'</svg></div></div>';
	...
	$(".subContentBlock").each(function(i, e) {
		$(e).attr("id", "card_" +i);
	});
...
function updatePlayerText(item,i){
	...
	var finalTally = item[item.length-1];
		...
		var targetClipHead = currCard+" .subSectionHeading";
		
		var targetClipNameColLeft = currCard+" .sub-left-col-copy";
		var rhColHtml = getMatchContents(item,i);	
		var lhColHtml = getLeftColContent(finalTally,item);
		$(targetClipHead).html(item[0].firstname+" "+item[0].scorer);
...
function getLeftColContent(finalTally,item){
	...
	//htmlStr +="<div class='playerCol-bg-img' style='background-image:url(images/"+item[0].scorer+".jpg)'></div>"+lhColCaptionHtml;
